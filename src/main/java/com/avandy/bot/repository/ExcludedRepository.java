...
public interface ExcludedRepository extends CrudRepository<Excluded, Long> {
    ...
    @Query(value = "SELECT lower(word) FROM excluded WHERE chat_id = :chatId", nativeQuery = true)
    List<String> findExcludedByChatId(Long chatId);

    @Query(value = "SELECT lower(word) FROM excluded WHERE chat_id = :chatId ORDER BY id DESC LIMIT 30", nativeQuery = true)
